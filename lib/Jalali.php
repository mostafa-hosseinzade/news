...
/** Hijri_Shamsi, Solar(Jalali) Date and Time public static functions
  -------------------------------------------------------
...
class Jalali {
    ...
    public static function jdate($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'fa') {
        ...
        $ts = $T_sec + (($timestamp == '' or $timestamp == 'now') ? time() : Jalali::tr_num($timestamp));
        $date = explode('_', date('H_i_j_n_O_P_s_w_Y', $ts));
        list($j_y, $j_m, $j_d) = Jalali::gregorian_to_jalali($date[8], $date[3], $date[2]);
        ...
        for ($i = 0; $i < $sl; $i++) {
            ...
            switch ($sub) {
                ...
                case'D':
                    $out.=Jalali::jdate_words(array('kh' => $date[7]), ' ');
                    break;

                case'f':
                    $out.=Jalali::jdate_words(array('ff' => $j_m), ' ');
                    break;

                case'F':
                    $out.=Jalali::jdate_words(array('mm' => $j_m), ' ');
                    break;
                ...
                case'J':
                    $out.=Jalali::jdate_words(array('rr' => $j_d), ' ');
                    break;

                case'k';
                    $out.=Jalali::tr_num(100 - (int) ($doy / ($kab + 365) * 1000) / 10, $tr_num);
                    break;

                case'K':
                    $out.=Jalali::tr_num((int) ($doy / ($kab + 365) * 1000) / 10, $tr_num);
                    break;

                case'l':
                    $out.=Jalali::jdate_words(array('rh' => $date[7]), ' ');
                    break;
                ...
                case'M':
                    $out.=Jalali::jdate_words(array('km' => $j_m), ' ');
                    break;
                ...
                case'p':
                    $out.=Jalali::jdate_words(array('mb' => $j_m), ' ');
                    break;
                ...
                case'q':
                    $out.=Jalali::jdate_words(array('sh' => $j_y), ' ');
                    break;
                ...
                case'r':
                    $key = Jalali::jdate_words(array('rh' => $date[7], 'mm' => $j_m));
                ...
                case'v':
                    $out.=Jalali::jdate_words(array('ss' => substr($j_y, 2, 2)), ' ');
                    break;

                case'V':
                    $out.=Jalali::jdate_words(array('ss' => $j_y), ' ');
                    break;
        ...
        return($tr_num != 'en') ? Jalali::tr_num($out, 'fa', '.') : $out;
    }
    ...
    public static function jstrftime($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'fa') {
        ...
        list($j_y, $j_m, $j_d) = Jalali::gregorian_to_jalali($date[7], $date[4], $date[3]);
        $doy = ($j_m < 7) ? (($j_m - 1) * 31) + $j_d - 1 : (($j_m - 7) * 30) + $j_d + 185;
        $kab = ($j_y % 33 % 4 - 1 == (int) ($j_y % 33 * .05)) ? 1 : 0;
        $sl = strlen($format);
        $out = '';
        for ($i = 0; $i < $sl; $i++) {
            ...
            switch ($sub) {
                ...
                /* Day */
                case'a':
                    $out.=Jalali::jdate_words(array('kh' => $date[6]), ' ');
                    break;

                case'A':
                    $out.=Jalali::jdate_words(array('rh' => $date[6]), ' ');
                    break;
                ...
                case'c':
                    $key = Jalali::jdate_words(array('rh' => $date[6], 'mm' => $j_m));
    ...
    public static function jmktime($h = '', $m = '', $s = '', $jm = '', $jd = '', $jy = '', $is_dst = -1) {
        ...
        if ($h == '' and $m == '' and $s == '' and $jm == '' and $jd == '' and $jy == '') {
            ...
            list($year, $month, $day) = Jalali::jalali_to_gregorian($jy, $jm, $jd);
    ...
    public static function jgetdate($timestamp = '', $none = '', $tz = 'Asia/Tehran', $tn = 'en') {
        $ts = ($timestamp == '') ? time() : Jalali::tr_num($timestamp);
        $jdate = explode('_', Jalali::jdate('F_G_i_j_l_n_s_w_Y_z', $ts, '', $tz, $tn));
        return array(
            'seconds' => Jalali::tr_num((int) Jalali::tr_num($jdate[6]), $tn),
            'minutes' => Jalali::tr_num((int) Jalali::tr_num($jdate[2]), $tn),
            'hours' => $jdate[1],
            'mday' => $jdate[3],
            'wday' => $jdate[7],
            'mon' => $jdate[5],
    ...
    public static function gregorian_to_jalali($gy, $gm, $gd, $mod = '') {
        if (strlen($gy) != 4 && strlen($gy) != 0 && strlen($gd) == 4 && strlen($gd) != 0) {
        $gd = $gy;
        $gy = $gd;
        }
        ...
        $gy = Jalali::tr_num($gy);
        $gm = Jalali::tr_num($gm);
        $gd = Jalali::tr_num($gd); /* <= Extra :اين سطر ، جزء تابع اصلي نيست */
    ...
    public static function jalali_to_gregorian($jy, $jm, $jd, $mod = '') {
        $jy = Jalali::tr_num($jy);
        $jm = Jalali::tr_num($jm);
        $jd = Jalali::tr_num($jd); /* <= Extra :اين سطر ، جزء تابع اصلي نيست */
        $gy = ($jy <= 979) ? 621 : 1600;
        $jy-=($jy <= 979) ? 0 : 979;
